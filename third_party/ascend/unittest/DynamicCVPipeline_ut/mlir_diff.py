...
def unified_diff(base_path, pr_path, rel_path):
    if is_binary(base_path) or is_binary(pr_path):
        return f"  Binary file differs: {rel_path}\n"
    lines_base = read_lines(base_path)
    pr_lines = read_lines(pr_path)
    diff = difflib.unified_diff(
        lines_base, pr_lines,
        fromfile=f'base/{rel_path}',
        tofile=f'pr/{rel_path}',
        lineterm=''
    )
    result = '\n'.join(diff)
    if result:
...
def compare_folders(folder_base, folder_pr, output_dir):
    ...
    for rel in common:
        ...
        if is_binary(full_name_base) or is_binary(full_name_pr):
            # skip binary file
            print(f" Encounter binary file, skipped: {rel}")
            ...
            try:
                ...
                if lines_base != lines_pr:
                    ...
                    diff_text = unified_diff(full_name_base, full_name_pr, rel)
                    if diff_text:
                        _write_diff_file(rel, diff_text, output_dir)
                else:
                    same_count += 1
...
def _write_diff_file(rel_path, content, output_dir):
    ...
    try: 
        ...
        print(f"diff file {diff_filename} written failed, exception is : {e}")
